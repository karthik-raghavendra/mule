...
public class AuthProvider implements AuthenticationProvider, MuleContextAware {
  ...
  private Map<String, String> callExternalAuthentication(String username, String password) {
    ...
    try {
        ...
        logger.info("Response from server was: " + response.getPayloadAsBytes());
        //logger.info("Response from mapper bytes was: " + mapper.readValue(response.getPayloadAsString(),new TypeReference<Map<String,String>>(){}));
        logger.info("Response from server was: " + response.getPayloadAsString());
